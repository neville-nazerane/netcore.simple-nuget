...
namespace NetCore.Simple
{
    ...
    [Route("[controller]/{id?}")]
    public abstract class CustomController<TModel, THandler> : AbstractController<TModel>
        where THandler : IModelHandler<TModel>, new()
    {
        ...
        protected override TModel Get(TModel model)
        {
            if (new THandler().GetBasicModel(model).Get())
                return model;
        ...
        protected override void OnAdd(TModel model) => new THandler().GetBasicModel(model).Add();

        protected override bool OnUpdate(TModel model) => new THandler().GetBasicModel(model).Update();

        protected override bool OnDelete(TModel model) => new THandler().GetBasicModel(model).Delete();
